...
const commentsByPostId: Record<number, Post2[]> = {
  1: [
    {
      id: 1,
      postId: 1,
...
function getPostById(id: number): Post | undefined {
  return posts2.find((post) => post.id2 === id);
}
...
function displayPost(): void {
  ...
  if (post) {
    ...
    if (commentsContainer) {
      const postComments = commentsByPostId[post.id2];
      ...
      if (postComments) {
        postComments.forEach((comment) => {
          ...
          commentElement.innerHTML = `
            <div id="comment-email">${comment.email}</div>
